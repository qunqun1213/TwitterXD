...
public class FanPage extends AppCompatActivity {
   // public static final String Web = "http://169.254.118.48:8080/Twitter/get_fan.jsp";
    public static final String Web = "http://192.168.43.228:8080/Twitter/get_fan.jsp";
    public RecyclerView mRecycleView;//定义RecyclerView
    //定义以Tongzhi实体类为对象的数据集合
    private ArrayList<Info> mdata = new ArrayList<Info>();
    //自定义recyclerveiw的适配器
    private Follow_in_Adapter fan_adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        refreshLayout = (SwipeRefreshLayout)findViewById(R.id.fansrl);
    ...
    private void initRecycleView() {
        //获取RecyclerView
        mRecycleView=(RecyclerView)findViewById(R.id.list_fan);
        //创建adapter
        fan_adapter = new Follow_in_Adapter(FanPage.this, mdata);
        //给RecyclerView设置adapter
        mRecycleView.setAdapter(fan_adapter);
        //设置layoutManager,可以设置显示效果，是线性布局、grid布局，还是瀑布流布局
        //参数是：上下文、列表方向（横向还是纵向）、是否倒叙
        mRecycleView.setLayoutManager(new LinearLayoutManager(FanPage.this, LinearLayoutManager.VERTICAL, false));
        //设置item的分割线
        mRecycleView.addItemDecoration(new DividerItemDecoration(FanPage.this,DividerItemDecoration.VERTICAL));
        mRecycleView.setItemAnimator (new DefaultItemAnimator());
        //RecyclerView中没有item的监听事件，需要自己在适配器中写一个监听事件的接口。参数根据自定义
        fan_adapter.setOnITemClickListener(new Follow_in_Adapter.OnITemClickListener() {
            @Override
...
//                            Intent intent = new Intent(FanPage.this,Homepage.class);
